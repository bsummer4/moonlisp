...
gen c = r 0 c where
	...
	r i (_,BLOCK(pre,post) [s]) = pre ++ r i (space s) ++ post
	r i (_,BLOCK(pre,post) mid) = pre ++ "\n" ++ block (1+i) mid ++ dent (i+1) post

atom x = (Space,ATOM x)
block d cs = (Space,BLOCK d cs)
blockexp d cs = (Unsafe,BLOCK d cs)
...
stmt s c = (Space,STMT s c)
delim (Safe,e) = (Safe,e)
delim (Unsafe,e) = (Safe,TUPLE ("(",")") [(Unsafe,e)])
delim (Space,e) = (Safe,TUPLE ("(",")") [(Space,e)])
space (Safe,e) = (Safe,e)
space (Space,e) = (Safe,TUPLE (" "," ") [(Unsafe,e)])
space (Unsafe,e) = (Safe,TUPLE ("(",")") [(Space,e)])
